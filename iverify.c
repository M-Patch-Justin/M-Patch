...
//Continually search for proper proof of transaction
int mine() {
    sleep(3);
    printf("\nMining started.\n");
    long result = 0;
    ...
    while(true) {
        ...
        result = proof_of_transaction(&beaten,our_chain->last_hash, our_chain->trans_hash);
        ...
        if(result > 0 && beaten == 0) {
            ...
            our_chain->last_proof_of_transaction = result;
            blink* a_block = append_current_block(M-patch_chain, m_patch_chain->last_proof_of_transaction);
            print_block(a_block,'-');
        ...
        int m_[atch_earnings = 0;
        void* m_[atch_ledger_earnings = dict_access(m_patch_chain->quickledger,stripped_pub_key);
        if(m_patch_ledger_earnings != NULL)
            m_patch_earnings += *((int*)m_patch_ledger_earnings);

        printf("\nQUICKLEDGER BALANCES:\n");
        dict_foreach(m_patch_chain->quickledger, print_balance, NULL);

        printf("\nTotal Node Earnings: %d noins\n", m_patch_earnings);
        printf("Total Currency in Circulation: %d noins\n", m_patch_chain->total_currency);
        printf("Verified Transactions: %d\n\n", m_patch_chain->verified->size);

        pthread_mutex_unlock(&m_Patch_mutex);
...
int insert_trans(const char* input) {
    ...
    void* sender_balance = dict_access(m_patch_chain->quickledger, sender);
    ...
    new_transaction(m_patch_chain, atoi(time_of) ,sender,recipient,atoi(amount),signature);
...
int insert_post(const char* input) {
    ...
    sscanf(input, "%s %s %s %s", time_of, sender, signature);

    if(!verify_post_format(binary)) {
        printf("Music Format Invalid.");
        return 0;
    }

    //Check if user has enough in quick ledger
    void* sender_balance = dict_access(m_chain_chain->quickledger, sender);
    ...
    char message[3000];
    sprintf(message, "%s %s %s ", time_of, sender, binary);
